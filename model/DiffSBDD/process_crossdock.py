...
except ImportError:  # BioPython >=1.83
...
def process_ligand_and_pocket(pdbfile, sdffile,
    ...
    # Keep coords and atom types on the SAME atoms (drop H if not in atom_dict).
    # RDKit >=2023: use GetSymbol(); Atom.element was removed.
    conf = ligand.GetConformer(0)
    lig_atoms = []
    lig_coords = []
    for atom in ligand.GetAtoms():
        sym = atom.GetSymbol()
        if sym.capitalize() in atom_dict or sym != 'H':
            lig_atoms.append(sym)
            lig_coords.append(list(conf.GetAtomPosition(atom.GetIdx())))
    ...
    assert len(lig_coords) == len(lig_one_hot), (
        f'ligand coords/types length mismatch ({sdffile})')
    ...
    if ca_only:
        ...
        others_idx = amino_acid_dict.get('others', len(amino_acid_dict) - 1)
        pocket_one_hot = []
        full_coords = []
        for res in pocket_residues:
            for atom in res.get_atoms():
                a = atom.element.strip()
                if a == 'H' and 'H' not in amino_acid_dict:
                    continue
                if a in amino_acid_dict:
                    idx = amino_acid_dict[a.capitalize()]
                else:
                    idx = others_idx
                pocket_one_hot.append(
                    np.eye(1, len(amino_acid_dict), idx).squeeze())
                full_coords.append(atom.coord)
        try:
            pocket_one_hot = np.stack(pocket_one_hot)
            full_coords = np.stack(full_coords)
        except ValueError as e:
            raise ValueError(f'{e} ({pdbfile}, {sdffile})') from e
        assert len(full_coords) == len(pocket_one_hot), (
            f'pocket coords/types length mismatch ({pdbfile})')
        pocket_data = {
            'pocket_coords': full_coords,
            'pocket_one_hot': pocket_one_hot,
            'pocket_ids': pocket_ids

...
class ProcessedLigandPocketDataset(Dataset):
    def __init__(self, npz_path, center=True, transform=None):
        ...
        # Drop examples where coords/types were misaligned during processing
        # (e.g. H atoms kept in coords but dropped from one-hot).
        n = len(self.data['names'])
        valid = []
        for i in range(n):
            try:
                if (len(self.data['lig_coords'][i])
                        == len(self.data['lig_one_hot'][i])
                        == len(self.data['lig_mask'][i])
                        and len(self.data['pocket_coords'][i])
                        == len(self.data['pocket_one_hot'][i])
                        == len(self.data['pocket_mask'][i])):
                    valid.append(i)
            except Exception:
                continue

        if len(valid) < n:
            print(f'{npz_path}: dropped {n - len(valid)}/{n} '
                  f'mismatched pocket–ligand examples')
            for k, v in list(self.data.items()):
                if isinstance(v, list):
                    self.data[k] = [v[i] for i in valid]
                elif torch.is_tensor(v) and v.numel() == n:
                    self.data[k] = v[valid]

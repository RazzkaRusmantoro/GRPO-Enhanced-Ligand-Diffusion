...
def make_mol_openbabel(positions, atom_types, atom_decoder):
    ...
    # Windows: NamedTemporaryFile stays locked while open, and Open Babel's
    # WriteFile often returns false / fails to replace an empty SDF. Convert
    # XYZ -> SDF in memory and let RDKit parse the molblock.
    xyz_fd, xyz_file = tempfile.mkstemp(suffix=".xyz")
    os.close(xyz_fd)
    try:
        utils.write_xyz_file(positions, atom_types, xyz_file)
        obConversion = openbabel.OBConversion()
        obConversion.SetInAndOutFormats("xyz", "sdf")
        ob_mol = openbabel.OBMol()
        if not obConversion.ReadFile(ob_mol, xyz_file):
            with open(xyz_file, encoding="utf-8") as handle:
                xyz_text = handle.read()
            if not obConversion.ReadString(ob_mol, xyz_text):
                raise OSError(f"Open Babel could not read generated XYZ {xyz_file}")
        sdf_text = obConversion.WriteString(ob_mol) or ""
        tmp_mol = Chem.MolFromMolBlock(sdf_text, sanitize=False, removeHs=False)
        if tmp_mol is None:
            raise OSError("RDKit could not parse Open Babel SDF molblock")
    finally:
        try:
            os.unlink(xyz_file)
        except OSError:

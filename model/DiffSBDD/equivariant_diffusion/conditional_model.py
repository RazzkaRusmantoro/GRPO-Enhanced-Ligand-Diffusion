...
class ConditionalDDPM(EnVariationalDiffusion):
    ...
    def sample_p_zs_given_zt(self, s, t, zt_lig, xh0_pocket, ligand_mask,
                             pocket_mask, fix_noise=False, guidance_fn=None):
        ...
        # Neural net prediction.  Keep the original inference path byte-for-byte
        # equivalent when guidance is disabled.  Guided calls locally override
        # sample_given_pocket's no-grad context so gradients exist only for the
        # selected reverse steps.
        guidance_active = guidance_fn is not None
        if guidance_active and hasattr(guidance_fn, 'is_active'):
            guidance_active = bool(guidance_fn.is_active(t))
        if not guidance_active:
            ...
            with torch.enable_grad():
                zt_for_guidance = zt_lig.detach().requires_grad_(True)
                eps_for_guidance, _ = self.dynamics(
                    zt_for_guidance, xh0_pocket.detach(), t,
                    ligand_mask, pocket_mask)
                denoised_lig = self.xh_given_zt_and_epsilon(
                    zt_for_guidance, eps_for_guidance, gamma_t, ligand_mask)
                guidance_delta = guidance_fn(
                    latent=zt_for_guidance,
                    denoised=denoised_lig,
                    pocket=xh0_pocket.detach(),
                    timestep=t,
                    previous_timestep=s,
                    ligand_mask=ligand_mask,
                    pocket_mask=pocket_mask,
                    transition_sigma=(
                        sigma_t_given_s * sigma_s / sigma_t).detach())
            eps_t_lig = eps_for_guidance.detach()
        ...
        if guidance_delta is not None:
            if isinstance(guidance_delta, dict):
                guidance_delta = guidance_delta.get('delta')
            if guidance_delta is not None:
                if guidance_delta.shape != mu_lig.shape:
                    raise ValueError(
                        'guidance_fn must return a delta with the ligand latent '
                        f'shape {tuple(mu_lig.shape)}, got '
                        f'{tuple(guidance_delta.shape)}')
                mu_lig = mu_lig + guidance_delta.detach()
    ...
    def sample_given_pocket(self, pocket, num_nodes_lig, return_frames=1,
                            timesteps=None, guidance_fn=None):
        """
...
class SimpleConditionalDDPM(ConditionalDDPM):
    ...
    def sample_given_pocket(self, pocket, num_nodes_lig, return_frames=1,
        ...
        return super(SimpleConditionalDDPM, self).sample_given_pocket(
            pocket, num_nodes_lig, return_frames, timesteps, guidance_fn)

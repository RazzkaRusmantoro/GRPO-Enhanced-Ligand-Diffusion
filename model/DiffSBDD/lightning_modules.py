...
class LigandPocketDDPM(pl.LightningModule):
    ...
    def generate_ligands(self, pdb_file, n_samples, pocket_ids=None,
                         ...
                         n_nodes_bias=0, n_nodes_min=0, guidance_fn=None,
                         **kwargs):
        """
        ...
        Args:
            ...
            guidance_fn: optional reverse-step callback. It is passed through
                unchanged to ConditionalDDPM.sample_given_pocket.
        ...
        elif isinstance(self.ddpm, ConditionalDDPM):
            xh_lig, xh_pocket, lig_mask, pocket_mask = \
                self.ddpm.sample_given_pocket(pocket, num_nodes_lig,
                                              timesteps=timesteps,
        ...
        for mol_pc in zip(utils.batch_to_list(x, lig_mask),
            ...
            try:
                mol = build_molecule(
                    *mol_pc, self.dataset_info, add_coords=True)
                ...
                # A single invalid Open Babel reconstruction should not discard
                # every other candidate in a sanitizing generation batch.
                if not sanitize:
                    raise
                mol = None
            if mol is not None:
